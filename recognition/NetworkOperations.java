...
public class NetworkOperations {
    ...
    private Nabla backpropogate(double[][] example) {
        ...
        FeedForwardResult feed = feedForward(Arrays.copyOf(example, 27));

        double[] cdVec = costDerivativeVec(example[27][0], feed.activations[feed.activations.length - 1]);
    ...
    void evaluate(ArrayList<double[][]> testData) {
        ...
        for (double[][] data : testData) {
            FeedForwardResult thisResult = feedForward(Arrays.copyOf(data, 27));

            if (thisResult.classification == data[27][0]) {
                numCorrect++;
            }
        }
